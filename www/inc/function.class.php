...
$strSQL = "select iscopy,otherheader,title,keywords,description,icp,mapcode,statistics,weburl,newsnum,productnum,bannertime,rmailbox as mail,smailbox as tel from siteset where id_siteset=1" ;
...
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select $tableField from productpic  where id_prodinfo ='".$pid."' order by id_prodpic asc limit 0,$num" ;
$objDB->Execute($strSQL);
...
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select opicname as pic from pagesetpic  where id_pageset ='".$lid."' order by  ordernum asc" ;
$objDB->Execute($strSQL);
