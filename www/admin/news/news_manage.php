...
//交换位置
	if(isset($_GET["action"]) && $_GET["action"]=="switch"){
		...
		$objDB->Execute($strSQL);
	    header('Location:news_manage.php?&searchselect='.$_GET[searchselect].'&searchdate=&keywords=&page='.$_GET[page]); exit();//跳转
	}
...
//输出一级目录  
  $strSQL="SELECT id_newsdir,name FROM newsdir where dele='1' and level='1' and  id_newsdir!='3' ";
  $objDB->Execute($strSQL);
...
$strSQL = "SELECT newsinfo.id_newsinfo,newsdir.name,newsinfo.newsdate,newsinfo.title,newsinfo.content,newsinfo.ordernum,newsinfo.id_newsdir,newsdir.lang FROM newsinfo left join newsdir on newsinfo.id_newsdir=newsdir.id_newsdir WHERE newsinfo.dele='1' $str order by newsinfo.ordernum DESC" ;
            ...
            <tr>
              ...
              <table width="100%" border="0" cellspacing="5" cellpadding="3">
                  <tr>
                    ...
                    <td width="51%" align="left"><a href="news_manage.php?03<?php echo $arrnewsinfos[$i][id_newsinfo];?>&searchselect=<?php echo $searchselect;?>&searchdate=<?php echo $searchdate;?>&keywords=<?php echo $keywords;?>"  class="txt_leftmenu"><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></a></td>
                    <td width="12%" align="center"><a href="javascript:void(0)" ><img src=getqrcode.php?nid=<?=$arrnewsinfos[$i][id_newsinfo];?>&ndir2=<?=$arrnewsinfos[$i][id_newsdir];?>&lang=<?=$arrnewsinfos[$i][lang];?>&ecc=Q&size=1   onclick="$.wholepop.dialogClass ='style_1';popmessage('<img src=getqrcode.php?nid=<?=$arrnewsinfos[$i][id_newsinfo];?>&ndir2=<?=$arrnewsinfos[$i][id_newsdir];?>&lang=<?=$arrnewsinfos[$i][lang];?>&ecc=Q&size=4  />','<?php echo cutstr($arrnewsinfos[$i][title],30,1);?>')"   /></a></td>
...
<a href='news_manage.php?action=switch&id=<?=$arrnewsinfos[$i][id_newsinfo];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i-1][id_newsinfo];?>&swtorder=<?=$arrnewsinfos[$i-1][ordernum];?>&page=<?=$_GET[page];?>&searchselect=<?=$_GET[searchselect];?>'><img src='../inc/pics/icon_001.gif' border='0' width='10' height='10'></a>&nbsp;
			<? }else {?>
<img src='../inc/pics/icon_0012.gif' border='0' width='10' height='10'>&nbsp;
			 
			<? } if($i!=$intNews-1){//判断一级目录是否位置在最后?>
<a href='news_manage.php?action=switch&id=<?=$arrnewsinfos[$i][id_newsinfo];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i+1][id_newsinfo];?>&swtorder=<?=$arrnewsinfos[$i+1][ordernum];?>&page=<?=$_GET[page];?>&searchselect=<?=$_GET[searchselect];?>'><img src='../inc/pics/icon_002.gif' border='0' width='10' height='10'></a>&nbsp;
			<? } else{ ?>
          ...
          <td align="center"><table width="93%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  ...
                  <tr bgcolor="#FFFFFF">
                    <td valign="top" bgcolor="#FFFFFF">信息标签</td>
                    <td bgcolor="#FFFFFF"><textarea name="tag" id="tag" style="width:500px" rows="2"><?php echo $onenewsinfo[tag];?></textarea></tr>
                  <tr bgcolor="#FFFFFF">
                  ...
                  <tr bgcolor="#FFFFFF">
                    ...
                    <td bgcolor="#FFFFFF"><span onclick="needupfile(<?php echo $onenewsinfo[id_newsinfo];?>,'../../upload/news/','|jpg|jpeg|pdf|rar|doc|gif|png|mp3','/admin/news/ajax_upfileindata.php');" style="cursor:pointer">文件上传...</span>
 <div id="pickuang">
<?php for($i=0;$i<sizeof($onenewspicinfo);$i++){
if($onenewspicinfo[$i][type]=="JPG" || $onenewspicinfo[$i][type]=="JPEG" || $onenewspicinfo[$i][type]=="GIF"  || $onenewspicinfo[$i][type]=="PNG" || $onenewspicinfo[$i][type]=="MP3"){
?>
