...
//交换位置
	if(isset($_GET["action"]) && $_GET["action"]=="switch"){
		$strSQL="UPDATE pageset SET ordernum=".$_GET[swtorder]." WHERE id_pageset=".$_GET[id]."";
		$objDB->Execute($strSQL);
		$strSQL="UPDATE pageset SET ordernum=".$_GET[order]." WHERE id_pageset=".$_GET[swtid]."";	
		$objDB->Execute($strSQL);
...
$strSQL = "SELECT * FROM pageset order by ordernum DESC" ;
...
if(isset($action) && $action=="02" && $onuserperm_addprem=='1'){
	...
	$strSQL="INSERT INTO pageset(title,pagetitle,keywords,description,content,intro,lang,location)
	                     values('".$_POST[title]."','".$_POST[pagetitle]."','".$_POST[keywords]."','".$_POST[description]."','".$_POST[newscontent]."','".$_POST[intro]."','".$_POST[lang]."','".$_POST[location]."')";
	$objDB->Execute($strSQL);
	
	$ordernumid=$objDB->getInsertID();//获取插入到数据库记录的ID号
	$strSQL="UPDATE pageset SET ordernum='$ordernumid'   where id_pageset='$ordernumid'";
	$objDB->Execute($strSQL);
...
if(isset($action) && substr($action,0,2)=="04" && $onuserperm_editprem=='1'){
    ...
    $strSQL="UPDATE pageset SET title='".$_POST[title]."',pagetitle='".$_POST[pagetitle]."',keywords='".$_POST[keywords]."',description='".$_POST[description]."',content='".$_POST[newscontent]."',intro='".$_POST[intro]."',lang='".$_POST[lang]."',location='".$_POST[location]."' where id_pageset=$onenewsinfoid";
	$objDB->Execute($strSQL);
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td align="center"><table width="95%" border="0" cellspacing="0" cellpadding="5">
            ...
            <tr>
              <td width="28%" height="30" align="left" valign="bottom" class="txt_leftmenu"><a href="pageset.php?03<?php echo $arrnewsinfos[$i][id_pageset];?>"  class="txt_leftmenu"><strong><?php echo cutstr($arrnewsinfos[$i][title],60,1);?></strong></a></td>
              <td width="51%" align="left" valign="bottom" class="txt_3">
              <?
              if($arrnewsinfos[$i][location]=='0'){echo '关闭';};
			  if($arrnewsinfos[$i][location]=='1'){echo '显示到所有页面';};
			  if($arrnewsinfos[$i][location]=='2'){echo '只显示到首页';};
			  if($arrnewsinfos[$i][location]=='3'){echo '显示到公司简介内页';};
			  ?></td>
...
<a href='pageset.php?action=switch&id=<?=$arrnewsinfos[$i][id_pageset];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i-1][id_pageset];?>&swtorder=<?=$arrnewsinfos[$i-1][ordernum];?>'><img src='../inc/pics/icon_001.gif' border='0' width='10' height='10'></a>&nbsp;
			<? }else {?>
<img src='../inc/pics/icon_0012.gif' border='0' width='10' height='10'>&nbsp;
			 
			<? } if($i!=$intNews-1){//判断一级目录是否位置在最后?>
<a href='pageset.php?action=switch&id=<?=$arrnewsinfos[$i][id_pageset];?>&order=<?=$arrnewsinfos[$i][ordernum];?>&swtid=<?=$arrnewsinfos[$i+1][id_pageset];?>&swtorder=<?=$arrnewsinfos[$i+1][ordernum];?>'><img src='../inc/pics/icon_002.gif' border='0' width='10' height='10'></a>&nbsp;
			<? } else{ ?>
...
<div id="lineout">
            ...
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  <tr bgcolor="#FFFFFF">
                    ...
                    <td width="404" bgcolor="#FFFFFF"><input name="title" type="text" class="input_01" id="title" style="width:400px" value="" /></td>
                    </tr>
                                        <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                    </tr>
                        <tr bgcolor="#FFFFFF">
                                          <td valign="top" bgcolor="#FFFFFF">版面语言</td>
                    ...
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                        ...
                        <tr bgcolor="#FFFFFF">
                          <td valign="top" bgcolor="#FFFFFF">显示位置</td>
                          <td bgcolor="#FFFFFF"><select name="location" id="location">
                            <option value="0">关闭</option>
                           <!--  <option value="1">显示到所有页面</option>-->
                         <option value="2">显示到首页</option>
                         <option value="3">显示到公司简介内页</option>
                          </select>                        
                        </tr>
                                            <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                    ...
                    <td bgcolor="#FFFFFF"><input name="pagetitle" type="text" class="input_01" id="pagetitle" style="width:400px" value="" />                      </tr>
                    <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                    </tr>
                  <tr bgcolor="#FFFFFF">
   ...
   $strSQL="SELECT opicname,id_pagesetpic,type,title FROM pagesetpic WHERE id_pageset=$onenewsid order by  id_pagesetpic asc";
   $objDB->Execute($strSQL);
            ...
            <tr>
              ...
              <table width="100%" border="0" cellpadding="4" cellspacing="0" class="txt">
                  <tr bgcolor="#FFFFFF">
                    ...
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                        ...
                        <tr bgcolor="#FFFFFF">
                                          <td valign="top" bgcolor="#FFFFFF">版面语言</td>
                                          <td bgcolor="#FFFFFF"><select name="lang" id="lang">
                                            <option value="1" <? if($onenewsinfo[lang]==1){echo 'selected';}?>>中文</option>
                                            <option value="0" <? if($onenewsinfo[lang]==0){echo 'selected';}?>>英文</option>
                                          </select>
                                            &nbsp;</tr>
                                                                <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                        ...
                        <tr bgcolor="#FFFFFF">
                          <td valign="top" bgcolor="#FFFFFF">显示位置</td>
                          <td bgcolor="#FFFFFF"><select name="location" id="location">
                           <option value="0"  <? if($onenewsinfo[location]==0){echo 'selected';}?>>关闭</option>
                          <!--  <option value="1"  <? if($onenewsinfo[location]==1){echo 'selected';}?>>显示到所有页面</option>-->
                            <option value="2"  <? if($onenewsinfo[location]==2){echo 'selected';}?>>显示到首页</option>
                          <? if($onenewsid!='20'){?>  <option value="3"  <? if($onenewsinfo[location]==3){echo 'selected';}?>>显示到公司简介内页</option> <? }//排除公司简介?>
                          </select>                        
                        </tr>
                                            <tr>
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                  ...
                  <tr bgcolor="#FFFFFF">
                    ...
                    <? if($onenewsid!='2' && $onenewsid!='19' && $onenewsid!='7'  && $onenewsid!='24'  && $onenewsid!='25'  ){//排除版面内容?>
                        <tr bgcolor="#FFFFFF">
                    <td valign="top" bgcolor="#FFFFFF">信息内容</td>
                    ...
                    <td colspan="3" align="left" valign="middle" bgcolor="#EBF3F9"  style="padding:0; height:1px;"></td>
                                            ...
                                            <tr bgcolor="#FFFFFF">
                                              ...
                                              <td bgcolor="#FFFFFF"><span onclick="needupfile(<?php echo $onenewsinfo[id_pageset];?>,'../../upload/layout/','|jpg|jpeg|gif|pdf|png|doc','/admin/siteset/ajax_upfilepagesetindata.php');" style="cursor:pointer">文件上传...</span>
                                              <div id="pickuang">
...
?>
<div class="box floatLeft" style="position:relative;background-color:#154f79"><a href="javascript:void(0)"  onclick="$.wholepop.dialogClass ='style_1';poppic('/upload/layout/<?php echo $onenewspicinfo[$i][opicname];?>','图片预览及地址复制','<?php echo $onenewspicinfo[$i][id_pagesetpic];?>');" <? if($onenewspicinfo[$i][opicname]!='nopic.gif' && $onenewspicinfo[$i][title]!=''){?> onmouseover=$("#hidetitlepic<?=$i;?>").show(); onmouseout=$("#hidetitlepic<?=$i;?>").hide(); <? }?>><img src="/upload/layout/<?php echo $onenewspicinfo[$i][opicname];?>" width="50" height="50" border="0" /></a><span class="txt_underpic" onclick="delpic(<?php echo $onenewspicinfo[$i][id_pagesetpic];?>);" style="cursor:pointer">删除</span>
<div id="hidetitlepic<?=$i;?>" style="position:absolute;left:0px;top:0px;overflow:hidden;width:50px; height:25px;<?php if($onenewspicinfo[$i][opicname]=='nopic.gif'){echo 'display:block;';}else{echo 'display:none;';}?>z-index:100; background-color:#CCC"><?php echo $onenewspicinfo[$i][title];?></div>
</div>
